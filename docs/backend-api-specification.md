...
バックエンドAPIを確認するための仮画面を用意している。この画面は正式なフロントエンドではなく、バックエンドの結合確認とデモだけを目的とする。

実装内容、起動方法、制約、正式フロントへの置き換え・削除手順は [仮バックエンドデモUI](./temporary-backend-demo-ui.md) を参照すること。

```text
http://localhost:5173/backend-demo
```

API接続先は次の環境変数で変更できる。未設定時は `http://localhost:8080`。
